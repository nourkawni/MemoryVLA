...
The robotic arm (Panda) root/base link is fixed at world **`(-0.615, 0, 0)`** with identity orientation.
The RPY are **extrinsic XYZ Euler angles** and are **unwrapped for temporal continuity across frames (so they can exceed ±π)**, not bounded principal values.
